    ...
    public sealed class MathParser
    {
        ...
        private Expression _ParseOneOrMoreHighPriority(List<_Token> tokens, ref int tokenIndex)
        {
            ...
            while (true)
            {
                ...
                switch (token._tokenType)
                {
                    case _TokenType.Power: currentExpression = TakeToPower(tokens, ref tokenIndex, currentExpression); break;
        ...
        private Expression TakeToPower(List<_Token> tokens, ref int tokenIndex, Expression currentExpression)
        {
            var expression = _GetValue(tokens, ref tokenIndex);
            Expression<Func<BigDecimal>>? func = Expression.Lambda<Func<BigDecimal>>(expression);
            var compiled = func.Compile();
            BigDecimal result = compiled.Invoke();

            if(result.IsNegative)
            {
                throw new ArgumentException("power can be negative");
            }

            Expression exp = currentExpression;
            for ( var i = 0; i < (int)result.ToBigInteger() - 1; i++)
            {
                exp = Expression.Multiply(exp, currentExpression);
            }
        ...
        private sealed class _Token
        {
            ...
            {
                return _tokenType.ToString() + ": " + _text;
            }
        }
        ...
        private static List<_Token> _ExtractTokens(string expression)
        {
            ...
            while (position < length)
            {
                ...
                switch (c)
                {
                    ...
                    case '^': result.Add(new _Token(_TokenType.Power, "^", position)); break;
